...
class CsvMcpServer {
    ...
    setupToolHandlers() {
        this.server.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
            ...
            try {
                switch (name) {
                    case "read_csv":
                        return await this.handleReadCsv(args || {});
                    case "read_csv_range":
                        return await this.handleReadCsvRange(args || {});
                    case "write_csv":
                        return await this.handleWriteCsv(args || {});
                    case "add_csv_row":
                        return await this.handleAddCsvRow(args || {});
                    case "delete_csv_row":
                        return await this.handleDeleteCsvRow(args || {});
                    case "update_csv_row":
                        return await this.handleUpdateCsvRow(args || {});
                    case "query_csv":
                        return await this.handleQueryCsv(args || {});
                    case "get_read_log":
                        return await this.handleGetReadLog(args || {});
                    case "clear_read_log":
                        return await this.handleClearReadLog(args || {});
                    case "list_read_logs":
                        return await this.handleListReadLogs(args || {});
                    default:
    ...
    async getReadLogInfo(csvFilePath) {
    ...
    async saveReadLog(csvFilePath, startRow, endRow, totalRows) {
        const logFile = this.getLogFilePath(csvFilePath);
            ...
            for (const logFile of logFiles) {
                if (logFile.endsWith('.json')) {
                    try {
                        ...
                        const logInfo = JSON.parse(logContent);
                        logs.push({
